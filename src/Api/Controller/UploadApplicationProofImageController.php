...
/**
 * 接收用户提交的"申请证明材料"图片上传,保存到 storage(后台可配置子目录)并在
 * achievement_images 表登记(记录上传者),返回图片服务路由 URL。
...
class UploadApplicationProofImageController implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        // 登记图片资源:记录上传者/原始文件名/MIME/大小,便于追溯与管理
        $image = AchievementImage::create([
            'type' => AchievementImage::TYPE_PROOF,
            'path' => $name,
        ...
        return new JsonResponse(['url' => '/achievement-images/'.$image->id, 'id' => $image->id], 201);
    }
}
